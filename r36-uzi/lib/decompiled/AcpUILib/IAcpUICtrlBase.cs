// Decompiled with JetBrains decompiler
// Type: AcpUILib.IAcpUICtrlBase
// Assembly: AcpUILib, Version=23.1.0.23, Culture=neutral, PublicKeyToken=d124d6ba4931c72b
// MVID: 6F9E0BB6-7C96-4FCD-AD4F-151E6FD8321B
// Assembly location: F:\DECOMP\36-UZI\APXFamilyCPS1\lib_org\AcpUILib.dll

#nullable disable
namespace AcpUILib;

public interface IAcpUICtrlBase
{
  bool IsAcpValid { get; set; }

  bool IsAcpApplicable { get; set; }

  bool IsAcpEditable { get; set; }

  bool IsAcpVisible { get; set; }
}
