...
public partial class WriteProtect : PageFunction<string>, INotifyPropertyChanged, IComponentConnector
{
  ...
  public static bool readInProgress = false;
  private bool queryRadioBtnWasClicked = false;
  private bool updateRadioDataBtnWasClicked = false;
  private int readTransportValue = 0;
  private Dictionary<string, AcpExpander> expanderlist = new Dictionary<string, AcpExpander>();
  private bool openFromRMC = false;
  private ObservableCollection<bool> isOwnerSystemIDSupported = (ObservableCollection<bool>) null;
    ...
    ObservableCollection<bool> isOwnerSystemIDSupported = null)
  {
    ...
    SystemKeyData systemKeyData2 = new SystemKeyData(KeySource.HARDWARE, "0", 0, KeyType.ADVANCED_SYSTEM_KEY, false, false, (AccessRecord) null);
  ...
  private void OnLoad(object sender, RoutedEventArgs e)
  {
    ...
    if (!string.IsNullOrEmpty(this.parentWindowSettings.UserSelectedApplicationLanguage))
      ...
      this.initialLanguageSelection = !AppResources.Culture.Name.Contains("es") ? AppResources.Culture.Name : AppResources.Culture.Name.Substring(0, 2);
  }
  ...
  private void buttonQuery_Click(object sender, RoutedEventArgs e)
  {
    ...
    try
    {
      ...
      if (this.attachedKeys != null && this.attachedKeys.Count > 0)
      {
        ...
        foreach (SystemKeyData attachedKey in this.attachedKeys)
        {
          if (attachedKey.Type == KeyType.ADVANCED_SYSTEM_KEY && attachedKey.SystemID == 1)
          {
            this.attachedKeys.Remove(attachedKey);
            break;
          }
        }
        this.attachedKeys.Insert(0, systemKeyData3);
        ...
        if (this.attachedASKs != null && this.attachedASKs.Count > 0)
        {
          ...
          else
          {
            ...
            this.accessReadWriteDevice.InitDocument();
  ...
  private void onHelpButton_Click(object sender, RoutedEventArgs e)
  {
    try
    {
      ...
      if (this.openFromRMC)
        AcpUI.Help.Utility.DisplayCPSHelp("zzRadioManagement\\QueryUpdateRadioDataWindow\\The_Query_Update_Radio_Data_Page_RadioWriteProtectRM.htm");
      else
        AcpUI.Help.Utility.DisplayCPSHelp("zzRadioWriteProtect\\RibbonCodeplugQueryUpdateRadioData\\The_Query_Update_Radio_Data_Page_RadioWriteProtect.htm");
    }
    catch (Exception ex)
    {
    }
  ...
  {
    if (this.comboDlgOwnerSystemIDChoices.SelectedValue != null && ((SystemKeyData) this.comboDlgOwnerSystemIDChoices.SelectedValue).SystemID != 0)
    {
    ...
    try
    {
      ...
      if (!RadioOperationValidator.IsRadioConvOnly(device.ModelNumber, device.AstroPurchasedFlashCode, false))
      {
        ...
        try
        {
          ...
          codeplug = ServerAccessor.GetInstance().GetCodeplug(radioByDeviceUuid.WorkingCodeplugUuid.Value) as APXCodeplug;
        }
        catch (Exception ex)
        {
          throw;
        }
        int? nullable = codeplug.OwnerAdvKeyType;
        string str = (nullable.GetValueOrDefault() != 0 ? 0 : (nullable.HasValue ? 1 : 0)) == 0 ? AcgResources.ID_ADVANCEDWACNKEY : AcgResources.ID_ADVANCEDSYSTEMKEY;
        this.boolDlgWriteProtect.IsChecked = codeplug.DisableWriteProtect;
        if (str == AcgResources.ID_ADVANCEDSYSTEMKEY)
        {
          nullable = codeplug.HomeSystemId;
          this.sysKeyDataFromRadio = new SystemKeyData(KeySource.HARDWARE, "0", nullable.Value, KeyType.ADVANCED_SYSTEM_KEY, false, false, (AccessRecord) null);
        }
        else if (str == AcgResources.ID_ADVANCEDWACNKEY)
        {
          nullable = codeplug.OwnerWacnId;
          this.sysKeyDataFromRadio = new SystemKeyData(KeySource.HARDWARE, "0", nullable.Value, KeyType.ADVANCED_WACN_KEY, false, false, (AccessRecord) null);
        }
        bool flag = false;
        SystemKeyData systemKeyData1 = new SystemKeyData(KeySource.HARDWARE, "0", 0, KeyType.NONE, false, false, (AccessRecord) null);
        foreach (SystemKeyData attachedAsK in (Collection<SystemKeyData>) this.attachedASKs)
          ...
          if (radioInformation.Labtool.RadInfoLabtoolQA01648ASKEnable_A37150Value)
          {
            ...
            if (RadioAccessValidatorBase.CheckIfOwnerASKIsAttached(homeSysId, keyTypeA38663Value, this.attachedKeys, out outASK))
            {
              if (outASK.AccessLevelType == KeyAccessLevelType.UNLM_ACC || outASK.AccessLevelType == KeyAccessLevelType.UNLM_ACC_WITHOUT_WP)
              {
                ...
                askRequiredA37152.SetValue(num3 != 0);
        ...
        if (!RadioOperationValidator.IsRadioConvOnly(astroRadio1.ModelNumber, astroRadio1.AstroPurchasedFlashCode, false))
        {
          ...
          if (RadioAccessValidatorBase.CheckIfOwnerASKIsAttached(homeSysId, homeKeyType, this.attachedKeys, out outASK))
          {
            if (outASK.AccessLevelType == KeyAccessLevelType.UNLM_ACC || outASK.AccessLevelType == KeyAccessLevelType.UNLM_ACC_WITHOUT_WP)
            {
              ...
              astroRadio2.AskRequired = num2 != 0;
              try
              {
              }
              catch (Exception ex)
