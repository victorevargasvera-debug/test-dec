...
public partial class Options : PageFunction<string>, INotifyPropertyChanged, IComponentConnector
{
  ...
  private void OnLoad(object sender, RoutedEventArgs e)
  {
    ...
    this.initialLanguageSelection = string.IsNullOrEmpty(this.parentWindowSettings.UserSelectedApplicationLanguage) ? (!AppResources.Culture.Name.Contains("es") && !AppResources.Culture.Name.Contains("ar") ? AppResources.Culture.Name : AppResources.Culture.Name.Substring(0, 2)) : this.parentWindowSettings.UserSelectedApplicationLanguage;
  ...
  private void buttonOK_Click(object sender, RoutedEventArgs e)
  {
    ...
    if (this.initialLanguageSelection != selectedItem || this.initialLanguageSelection != AppResources.Culture.NativeName)
    {
      ...
      {
        this.parentWindowSettings.SelectedHelpLanguage_RootDir = string.Format("Help{0}{1}{0}FlashHelp{0}", (object) Path.DirectorySeparatorChar, (object) str);
        this.parentWindowSettings.UserSelectedApplicationLanguage = str;
      }
      else
      {
        this.parentWindowSettings.SelectedHelpLanguage_RootDir = string.Format("Help{0}{1}{0}FlashHelp{0}", (object) Path.DirectorySeparatorChar, (object) App.AdditionalCPSLanguages[selectedItem]);
  ...
  private void onHelpButton_Click(object sender, RoutedEventArgs e)
  {
    try
    {
      ...
      AcpUI.Help.Utility.DisplayCPSHelp("zzCPSOptions\\zWinTopic\\The_Options_Window_Options.htm");
    }
    catch (Exception ex)
    {
    }
  ...
  {
    string str = this.m_ARSAppInstallPath + "\\SSLAdminTool.exe";
  ...
  {
    string str = this.m_AKAAppInstallPath + "ASKAdminTool.exe";
    Process process = new Process();
    process.StartInfo.FileName = str;
    try
    {
  ...
  public bool IsLauageButtonVisible => false;
  ...
  internal bool IsARSAdminAppInstalled
  {
    get
    {
      ...
      string name = "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{D627D6DB-5F0E-4BA9-99D7-19AFE9A9536B}";
      try
      {
        this.m_ARSAppInstallPath = Registry.LocalMachine.OpenSubKey(name).GetValue("InstallLocation", (object) string.Empty).ToString();
        if (this.m_ARSAppInstallPath.Equals(string.Empty))
          adminAppInstalled = false;
        else if (!File.Exists(this.m_ARSAppInstallPath + "\\SSLAdminTool.exe"))
          adminAppInstalled = false;
      }
      catch (Exception ex)
      {
  ...
  internal bool IsAKAAdminAppInstalled
  {
    ...
    {
      bool adminAppInstalled = true;
      string name = "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{A5718B72-72BC-4F12-AD16-F9DD422A5CD9}";
      try
      {
        this.m_AKAAppInstallPath = Registry.LocalMachine.OpenSubKey(name).GetValue("InstallLocation", (object) string.Empty).ToString();
        if (this.m_AKAAppInstallPath.Equals(string.Empty))
          adminAppInstalled = false;
        else if (!File.Exists(this.m_AKAAppInstallPath + "ASKAdminTool.exe"))
          adminAppInstalled = false;
      }
      catch (Exception ex)
      {
